    ...
    while True:
        ...
        if not won and not lost and not paused:
            ...
            if nowRun % difficulty == 0:
                nextMove = gameController.nextGhostMove(
                    blackGhost.currentPosition, (hero.x, hero.y))
                if nextMove not in ghostFound:
                    blackGhostNextMove = nextMove
                    ghostFound.append(nextMove)
                nextMove = gameController.nextGhostMove(
                    greenGhost.currentPosition, (hero.x, hero.y))
                if nextMove not in ghostFound:
                    greenGhostNextMove = nextMove
                    ghostFound.append(nextMove)
                nextMove = gameController.nextGhostMove(
                    whiteGhost.currentPosition, (hero.x, hero.y))
                if nextMove not in ghostFound:
                    whiteGhostNextMove = nextMove
                    ghostFound.append(nextMove)
                nextMove = gameController.nextGhostMove(
                    tillGhost.currentPosition, (hero.x, hero.y))
                if nextMove not in ghostFound:
            ...
            graphicsHandler.drawText(
                7, 18, "Target Score: " + str(gameController.targetScore))
        ...
        if not won:
            lost = lost or gameController.checkLoss(
                blackGhost.currentPosition, (hero.x, hero.y))
            lost = lost or gameController.checkLoss(
                greenGhost.currentPosition, (hero.x, hero.y))
            lost = lost or gameController.checkLoss(
            ...
            lost = lost or gameController.checkLoss(
                tillGhost.currentPosition, (hero.x, hero.y))
        if lost:
            graphicsHandler.displayLossScreen(
                (hero.x, hero.y), hero.score, hero.timeTaken)
